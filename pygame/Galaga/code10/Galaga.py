...
class Game():
    ...
    def create_boss(self):
        self.boss = Boss()
        self.boss_grp = pygame.sprite.GroupSingle(self.boss)
    ...
    def check_boss_collision(self):
        if self.laser_grp:
            if pygame.sprite.spritecollide(self.boss, self.laser_grp, True):
                self.boss.life -= 1
                self.score += 1
                if self.boss.life == 0:
                    self.game_status = "Logo"
    ...
    def show_bosslife(self):
        self.blive_suf = self.font.render(f"Boss Lives:{self.boss.life}", False, "white")
        self.blive_rec = self.level_suf.get_rect(center=(cfg.SCREEN_WIDTH//2, 20))                
        screen.blit(self.blive_suf, self.blive_rec)
    ...
    def run(self):
        if self.game_status == "Run":
            ...
            if self.level < 4:
                ...
                self.player_grp.update()
                self.laser_grp.update()
                self.boss_grp.update()
                self.show_bosslife()
                self.check_boss_collision()
                self.player_grp.draw(screen)
                self.laser_grp.draw(screen)
                self.boss_grp.draw(screen)
