...
def main():
    ...
    all_sprties.add(cat)
    all_sprties.add(ball)
    ball_sprites = pygame.sprite.Group()  #還要另外加一個sprite group
    all_sprties.add(ball)
    balls = []                            #跟一個ball的list
    balls.append(ball)
    ...
    while running:
        ...
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if timer==20:
                    ball2=Ball()
                    all_sprties.add(ball2)    #這裡把第二顆球加進去
    
        #計算
        all_sprties.update()                  #這裡改成這樣，一起update
    
        #更新畫面
        #背景
        screen.blit(bg_img, (0,0))
        all_sprties.draw(screen)              #這裡改成這樣，一起draw，雖然沒有draw函式
