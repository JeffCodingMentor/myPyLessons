...
import pygame
import math, random
import os                 #加入os，並改用os.path.join
...
class Ball(pygame.sprite.Sprite):
    ...
    def __init__(self):
        ...
        #加入籃球圖片
        self.raw_image = pygame.image.load(os.path.join("res","basketball.png")).convert_alpha()
...
class Cat(pygame.sprite.Sprite):
    ...
    def load_images(self):
        self.costumes = {}
        raw_image=pygame.image.load(os.path.join("res","cat.png")).convert_alpha()
        self.costumes["normal"]=pygame.transform.scale(raw_image, (95,100))
        raw_image = pygame.image.load(os.path.join("res","cat2.png")).convert_alpha()
        self.costumes["green"]=pygame.transform.scale(raw_image, (95,100))
        self.crnt_cstm = "normal"
        
    def __init__(self):
        super().__init__()
        #加入貓咪圖片
        self.load_images()  #先執行
        self.image = self.costumes[self.crnt_cstm]
        self.rect = self.image.get_rect()
    ...
    def update(self):
        if self.status == 1:
            ...
            if self.rect.centerx > SCREEN_WIDTH//2:
                self.image = pygame.transform.flip(self.costumes[self.crnt_cstm], True, False)
            else:
                self.image = self.costumes[self.crnt_cstm]
...
def main():
    ...
    bg_raw_image = pygame.image.load(os.path.join("res","background.png")).convert_alpha()
    bg_img = pygame.transform.scale(bg_raw_image, (SCREEN_WIDTH, SCREEN_HEIGHT))
    meow_snd = pygame.mixer.Sound(os.path.join("res","Meow.wav"))
    life_font = pygame.font.Font(os.path.join("res","arial.ttf"), 30)   #這裡有修改
    timer_font = pygame.font.Font(os.path.join("res","arial.ttf"), 30)  #這裡有修改
    ball = Ball()
    cat = Cat()
